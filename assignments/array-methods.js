...
runners.forEach(function(valueOf) {
    console.log(valueOf.first_name);
    console.log(valueOf.last_name);
  });
...
runners.map(function(valueOf){
    return valueOf.first_name.toUpperCase();
});
...
runners.filter(valueOf => {if (valueOf.shirt_size === 'L') {largeShirts.push(valueOf)}})
console.log(largeShirts)

// ==== Challenge 4: Use .reduce() ====
// The donations need to be tallied up and reported for tax purposes. Add up all the donations into a ticketPriceTotal array and log the result
const ticketPriceTotal = runners.reduce(function(summ, valueOf){
    console.log(`This is the amount of the accumulated donation ${summ}`);
    console.log(`this is the donation to add ${valueOf.donation}`);
    return summ + valueOf.donation;
},0);
...
console.log(runners.filter(valueOf => valueOf.first_name.startsWith('A')));

// Problem 2
const companyName=[];
runners.forEach(function(valueOf){
console.log(valueOf.company_name);
});
console.log(companyName);


// Problem 3
const generosity = runners.find(function(valueOf) {
    return valueOf.donation > 250;});
